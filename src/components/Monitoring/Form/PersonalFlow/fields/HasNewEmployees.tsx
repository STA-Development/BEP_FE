...
const HasNewEmployees = () => {
  const [t] = useTranslation()
  const fieldName = 'hasNewEmployees'
  const { control } = useFormContext()
  const { field } = useController({ name: fieldName, control })

  return (
    <div>
      <p>{t(Translation.PAGE_MONITORING_SYSTEM_FORM_ORGANIZATION_PERSONAL_NEW_EMPLOYEES)}</p>
...
export default HasNewEmployees
