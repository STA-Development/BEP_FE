...
const HasFiredWorkers = () => {
  const [t] = useTranslation()
  const fieldName = 'hasFiredWorkers'
  const { control } = useFormContext()
  const { field } = useController({ name: fieldName, control })

  return (
    <div>
      <p>{t(Translation.PAGE_MONITORING_SYSTEM_FORM_ORGANIZATION_PERSONAL_HAS_FIRED_WORKERS)}</p>
...
export default HasFiredWorkers
